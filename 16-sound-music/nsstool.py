...
def nss_to_asm(nss, m, fd):
    size = sum([len(astuple(op))-1+1 for op in nss])
    print(";;; Name: %s" % m.name, file=fd)
    print(";;; Author: %s" % m.author, file=fd)
...
def main():
    ...
    parser.add_argument("-n", "--name", default="",
                        help="Name of the ASM label for the NSS data. Unused by default")
