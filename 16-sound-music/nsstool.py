...
def convert_fm_row(row, channel, opcodes): 
    ...
    if not is_empty(row):
        ...
        if row.vol != -1:
            opcodes.append(fm_vol(tweak_fm_vol(row.vol)))
        # instrument
        if row.ins != -1:
...
def convert_s_row(row, channel, opcodes): 
    ...
    if not is_empty(row):
        ...
        # volume
        if row.vol != -1:
            opcodes.append(s_vol(tweak_s_vol(row.vol)))
        # effects
        for fx, fxval in row.fx:
            if fx == -1:      # empty fx
                pass
...
def convert_a_row(row, channel, opcodes): 
    ...
    if not is_empty(row):
        ...
        # volume
        if row.vol != -1:
            opcodes.append(a_vol(tweak_a_vol(row.vol)))
        # effects
        for fx, fxval in row.fx:
            if fx == -1:      # empty fx
                pass
...
def convert_b_row(row, channel, opcodes): 
    ...
    if not is_empty(row):
        ...
        # volume
        if row.vol != -1:
            opcodes.append(b_vol(tweak_b_vol(row.vol)))
        # effects
        for fx, fxval in row.fx:
            if fx == -1:      # empty fx
                pass
